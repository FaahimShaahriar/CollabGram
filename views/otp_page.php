...
include_once("../controller/userData.php");// Your database configuration
$con = getConnection();
...
if (isset($_POST['verify'])) {
    ...
    $otpQuery = "SELECT * FROM membership WHERE user_id = '$userId' AND otp = '$otpEntered' AND status = 'pending'";
    $result = mysqli_query($con, $otpQuery);

    // Check if any rows were returned
    if ($result && mysqli_num_rows($result) > 0) {
        // Update membership status to 'active'
        $updateQuery = "UPDATE membership SET status = 'active' WHERE user_id = '$userId' AND otp = '$otpEntered'";
